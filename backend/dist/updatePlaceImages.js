...
const updatePlaceImages = async () => {
    try {
        ...
        for (const [regionName, placeMapping] of Object.entries(mappings)) {
            const region = await Region_1.Region.findOne({ name: regionName });
            if (!region) {
            ...
            for (const [placeName, fileName] of Object.entries(placeMapping)) {
                ...
                const result = await Place_1.Place.updateOne({ regionId: region._id, name: placeName }, { $set: { imageUrl: imageUrl } });
